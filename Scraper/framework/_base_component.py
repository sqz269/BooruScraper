...
class BaseComponent(Utils, metaclass=Singleton):
    ...
    def init_config(self, config_path, load_config_from_abs_path):
        ...
        self.debug_print(f"[*] Loading configuration via: {'absolute path' if load_config_from_abs_path else 'configuration directory'}")
    ...
    def init_logger(self):
        self.debug_print(f"[*] Preparing Loggers; Logger name: {self.config['logger_name']}; File Level: {self.config['logger_file']}; Console Level: {self.config['logger_stdout']}")
